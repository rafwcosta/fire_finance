...
function Sel({ label, value, onChange, options }) {
  return (
    <div style={{ marginBottom:14 }}>
      <label style={G.label}>{label}</label>
      <select value={value} onChange={onChange} style={{ ...G.input, appearance:'none' }}>
      ...
      <div style={{ background:'#1a1f2e', borderRadius:'20px 20px 0 0', width:'100%',
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          ...
          <button onClick={onClose} style={{ background:'#252b3b', border:'none', borderRadius:8,
            color:'#aaa', padding:'6px 12px', cursor:'pointer', display:'flex' }}>
            <Ico.Close />
          </button>
        </div>
        {children}
...
function TelaResumo({ data, setData }) {
  ...
  const salvarConfig = () => {
    setData({ ...data, config: { ...form, limite: parseBRL(String(form.limite)) } })
    setEditando(false)
  }
          ...
          <div key={i} style={{ ...G.card, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ display:'flex', alignItems:'center', gap:12, flex:1 }}>
              <div style={{ width:8, height:8, borderRadius:'50%', flexShrink:0,
                background: item.valor < 0 ? '#ef4444' : item.valor === 0 ? '#4b5563' : catInfo.color }} />
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:16 }}>
        {[['Total',total,cor],['Pendente',pendente,'#f59e0b'],['Quitado',quitado,'#10b981']].map(([l,v,c]) => (
          <div key={l} style={{ ...G.card, margin:0, padding:'12px 14px', border:`1px solid ${c}25` }}>
            <p style={{ color:'#8892a4', fontSize:10, textTransform:'uppercase', margin:'0 0 4px' }}>{l}</p>
...
function TelaInvestimentos({ data, setData }) {
  ...
  return (
    <div style={{ padding:'0 16px 16px' }}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:16 }}>
        {[['Total',total,'#818cf8'],['Concluído',concluido,'#10b981'],['Pendente',pendente,'#f59e0b']].map(([l,v,c]) => (
          <div key={l} style={{ ...G.card, margin:0, padding:'12px 14px', border:`1px solid ${c}25` }}>
            <p style={{ color:'#8892a4', fontSize:10, textTransform:'uppercase', margin:'0 0 4px' }}>{l}</p>
            <p style={{ color:c, fontSize:15, fontWeight:700, margin:0 }}>{fmt(v)}</p>
          </div>
...
export default function App() {
  ...
  return (
    <div style={{ background:'#0f1420', minHeight:'100vh',
                  fontFamily:"'DM Sans',system-ui,sans-serif", color:'#cdd5e0' }}>
      <style>{`
      ...
      {menuBackup && (
        <Sheet title="Dados & Backup" onClose={() => setMenuBackup(false)}>
          ...
          <button onClick={exportar} style={{ ...G.btn('#3b82f6'), display:'flex',
            ...
            <Ico.Down /> Exportar backup (.json)
          </button>
          <label htmlFor="importFile" style={{ ...G.btn('#8b5cf6'), display:'flex',
            alignItems:'center', justifyContent:'center', gap:8, marginBottom:12,
